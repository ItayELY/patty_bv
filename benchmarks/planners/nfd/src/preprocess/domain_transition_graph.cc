...
void DomainTransitionGraph::addAxTransition(int from, int to, const Axiom_relational &ax,
                                            int ax_index) {
    Transition trans(to, ax_index);
    Condition &cond = trans.condition;
    for (const Axiom_relational::Condition &ax_cond : ax.get_conditions())
        if (true) // [cycles]
...
//    	cout << "Finalizing graph, " << vcount++ << " / " << vertices.size() << " vertices" << endl;
...
//        	if (DEBUG && j % 100 == 0) cout << "  " << j << " / " << num_transitions << " transitions" << endl;
            if (!is_dominated[j]) {
...
void build_DTGs(const vector<Variable *> &var_order,
    ...
    if (DEBUG) cout << " - DTG operators -"<<endl;
    ...
    if (DEBUG) cout << " - DTG axioms -"<<endl;
    for (size_t i = 0; i < axioms.size(); i++) {
        const Axiom_relational &ax = axioms[i];
    ...
    if (DEBUG) cout << " - DTG finalizing " << transition_graphs.size() << " graphs -"<<endl;
    int i = 0;
    for (auto &transition_graph : transition_graphs) {
        transition_graph.finalize();
        if (DEBUG) cout << " - progress: " << i++ << endl;
    }
    if (DEBUG) cout << " - DTG done -"<<endl;
}
