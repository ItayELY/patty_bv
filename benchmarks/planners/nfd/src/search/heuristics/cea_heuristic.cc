...
ap_float ContextEnhancedAdditiveHeuristic::compute_costs(const State &state) {
...
//                assert(!op.is_axiom()); //axioms are now handled in their own class
                set_preferred(op);
            }
        } else {
...
ap_float ContextEnhancedAdditiveHeuristic::compute_heuristic(const GlobalState &g_state) {
