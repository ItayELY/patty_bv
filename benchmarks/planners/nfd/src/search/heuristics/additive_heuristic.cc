...
ap_float AdditiveHeuristic::compute_add_and_ff(const State &state) {
...
ap_float AdditiveHeuristic::compute_heuristic(const State &state) {
    ap_float h = compute_add_and_ff(state);
    if (h != DEAD_END) {
        for (size_t i = 0; i < goal_propositions.size(); ++i)
...
ap_float AdditiveHeuristic::compute_heuristic(const GlobalState &global_state) {
