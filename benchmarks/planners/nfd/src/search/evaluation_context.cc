...
EvaluationContext::EvaluationContext(
    const HeuristicCache &cache, ap_float g_value, bool is_preferred,
...
EvaluationContext::EvaluationContext(
    const GlobalState &state, ap_float g_value, bool is_preferred,
    SearchStatistics *statistics, bool calculate_preferred)
...
ap_float EvaluationContext::get_g_value() const {
...
ap_float EvaluationContext::get_heuristic_value(ScalarEvaluator *heur) {
    ap_float h = get_result(heur).get_h_value();
    assert(h != EvaluationResult::INFTY);
    return h;
}

ap_float EvaluationContext::get_heuristic_value_or_infinity(ScalarEvaluator *heur) {
