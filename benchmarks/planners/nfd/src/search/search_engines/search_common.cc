...
static shared_ptr<OpenListFactory> create_alternation_open_list_factory(
    const vector<shared_ptr<OpenListFactory>> &subfactories, ap_float boost) {
    Options options;
...
static ScalarEvaluator *create_wastar_eval(
    GEval *g_eval, ap_float w, ScalarEvaluator *h_eval) {
    if (w == 0)
        return g_eval;
