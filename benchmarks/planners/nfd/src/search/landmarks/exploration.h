...
struct ExUnaryOperator {
    ...
    ExUnaryOperator(const std::vector<ExProposition *> &pre, ExProposition *eff,
                    const GlobalOperator *the_op, ap_float base)
...
class Exploration : public Heuristic {
    static constexpr ap_float MAX_COST_VALUE = 100000000; // See additive_heuristic.h.
    ...
    void enqueue_if_necessary(ExProposition *prop, ap_float cost, int depth, ExUnaryOperator *op,
                              bool use_h_max);
    void increase_cost(ap_float &cost, ap_float amount);
    void write_overflow_warning();
protected:
    virtual ap_float compute_heuristic(const GlobalState &state);
public:
