...
void LandmarkFactoryZhuGivan::extract_landmarks(
    ...
    for (size_t i = 0; i < g_goal.size(); ++i) {
        ...
        for (const auto & lml : goal_node.labels) {
            if (lml.first == g_goal[i].first && lml.second == (int) g_goal[i].second) // ignore label on itself
                continue;
            LandmarkNode *node;
            // Add new landmarks
            if (!lm_graph->simple_landmark_exists(lml)) {
                node = &lm_graph->landmark_add_simple(lml);

                // if landmark is not in the initial state,
                // relaxed_task_solvable() should be false
                assert((int) g_initial_state()[lml.first] == lml.second ||
                       !relaxed_task_solvable(true, node));
            } else
                node = &lm_graph->get_simple_lm_node(lml);
...
LandmarkFactoryZhuGivan::proposition_layer LandmarkFactoryZhuGivan::build_relaxed_plan_graph_with_labels() const {
    ...
    unordered_set<int> triggered(g_operators.size() + g_logic_axioms.size());
...
void LandmarkFactoryZhuGivan::compute_triggers() {
    ...
    // compute triggers
    for (size_t i = 0; i < g_operators.size() + g_logic_axioms.size(); ++i) {
