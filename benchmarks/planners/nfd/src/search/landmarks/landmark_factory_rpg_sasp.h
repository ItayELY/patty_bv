...
class LandmarkFactoryRpgSasp : public LandmarkFactory {
    ...
    void get_greedy_preconditions_for_lm(const LandmarkNode *lmp,
                                         const GlobalOperator &o, std::unordered_map<int, container_int> &result) const;
    void compute_shared_preconditions(std::unordered_map<int, container_int> &shared_pre,
