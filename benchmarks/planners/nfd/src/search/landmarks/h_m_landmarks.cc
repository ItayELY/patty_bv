...
void HMLandmarks::get_m_sets_(int m, int num_included, int current_var,
    ...
    for (int i = 0; i < (int) g_variable_domain[current_var]; ++i) {
        bool use_var = true;
...
void HMLandmarks::get_m_sets(int m,
    ...
    for (size_t i = 0; i < g_variable_domain.size(); ++i) {
        state_fluents.push_back(std::make_pair(i, (int) s[i]));
    }
...
void get_operator_precondition(int op_index, FluentSet &pc) {
    ...
    for (size_t i = 0; i < preconditions.size(); ++i)
        pc.push_back(make_pair(preconditions[i].var, (int) preconditions[i].val));
...
void get_operator_postcondition(int op_index, FluentSet &post) {
    ...
    for (size_t i = 0; i < effects.size(); ++i) {
        post.push_back(make_pair(effects[i].var, (int) effects[i].val));
        has_effect_on_var[effects[i].var] = true;
    }

    for (size_t i = 0; i < preconditions.size(); ++i) {
        if (!has_effect_on_var[preconditions[i].var])
            post.push_back(make_pair(preconditions[i].var, (int) preconditions[i].val));
    }
...
void HMLandmarks::generate_landmarks() {
    ...
    //NFD HACK: turn g_goal pair<int,container_int> into a vector of pair<int,int>
    for (const pair<int, container_int> &goalfluent : g_goal) {
    	goal_fluents.push_back(make_pair(goalfluent.first, (int) goalfluent.second));
    }
    get_m_sets(m_, goal_subsets, goal_fluents);
