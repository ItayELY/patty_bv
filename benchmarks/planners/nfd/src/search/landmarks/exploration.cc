...
Exploration::Exploration(const options::Options &opts)
    ...
    for (size_t var = 0; var < g_variable_domain.size(); ++var) {
        ...
        for (int val = 0; val < (int) g_variable_domain[var]; ++val) {
    ...
    for (size_t i = 0; i < g_axioms_as_operator.size(); ++i)
        build_unary_operators(g_axioms_as_operator[i]);
...
void Exploration::increase_cost(ap_float &cost, ap_float amount) {
    assert(cost >= 0);
    assert(amount >= 0);
    cost += amount;
...
void Exploration::build_unary_operators(const GlobalOperator &op) {
    ...
    ap_float base_cost = get_adjusted_cost(op);
    ...
    for (size_t i = 0; i < preconditions.size(); ++i) {
        ...
        assert(//preconditions[i].val >= 0 && // always true for container_int
        		preconditions[i].val < g_variable_domain[preconditions[i].var]);
    ...
    for (size_t i = 0; i < effects.size(); ++i) {
        ...
        assert(//effects[i].val >= 0 && // always true for container_int
        		effects[i].val < g_variable_domain[effects[i].var]);
        ...
        for (size_t j = 0; j < eff_conds.size(); ++j) {
            ...
            assert(//eff_conds[j].val >= 0 &&
            		eff_conds[j].val < g_variable_domain[eff_conds[j].var]);
...
void Exploration::setup_exploration_queue(const GlobalState &state,
    ...
    for (size_t i = 0; i < unary_operators.size(); ++i) {
        ...
        if (op.unsatisfied_preconditions == 0) {
            ...
            int depth = 1; // op.op->is_axiom() ? 0 : 1;
...
void Exploration::relaxed_exploration(bool use_h_max = false, bool level_out = false) {
    ...
    while (!prop_queue.empty()) {
        pair<ap_float, ExProposition *> top_pair = prop_queue.pop();
        ...
        for (size_t i = 0; i < triggered_operators.size(); ++i) {
            ...
            if (unary_op->unsatisfied_preconditions == 0) {
                int depth = //unary_op->op->is_axiom() ? unary_op->depth :
                		unary_op->depth + 1;
                if (use_h_max)
...
void Exploration::enqueue_if_necessary(ExProposition *prop, ap_float cost, int depth,
                                       ExUnaryOperator *op,
                                       bool use_h_max) {
    assert(cost >= 0);
...
int Exploration::compute_hsp_add_heuristic() {
    ...
    for (size_t i = 0; i < goal_propositions.size(); ++i) {
        ap_float prop_cost = goal_propositions[i]->h_add_cost;
        if (prop_cost == -1)
            return DEAD_END;
...
//        int offset = g_operators.size();
        for (size_t i = 0; i < g_logic_axioms.size(); ++i) {
            assert(false); // TODO handle this (axioms have a separate class now)
        ...
        for (size_t i = 0; i < unary_operators.size(); ++i) {
            ...
            ap_float new_lvl = op.h_max_cost - 1;
...
ap_float Exploration::compute_heuristic(const GlobalState &state) {
...
void Exploration::collect_ha(ExProposition *goal,
    ...
    if (unary_op) { // We have not yet chained back to a start node.
        ...
        const GlobalOperator *op = unary_op->op;
        bool added_to_relaxed_plan = false;
        // if (!op->is_axiom())   // axioms are handled separately now
            added_to_relaxed_plan = relaxed_plan.insert(op).second;
        if (added_to_relaxed_plan
            && unary_op->h_add_cost == unary_op->base_cost
            && unary_op->depth == 0
...
bool Exploration::plan_for_disj(vector<pair<int, int> > &landmarks,
    ...
    if (!landmarks.empty()) {
        ...
        for (size_t i = 0; i < termination_propositions.size(); ++i) {
            const ap_float prop_cost = termination_propositions[i]->h_add_cost;
