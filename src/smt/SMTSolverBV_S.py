from pysmt.logics import QF_LRA, QF_NRA, QF_BV
...
class SMTSolver:
    ...
    def getSolution(self) -> SMTSolution or bool:
        ...
        for variable in self.variables:
            ...
            if hasattr(value, "is_bv_constant") and value.is_bv_constant():
                solution.addVariable(variable, value.constant_value())

# Boolean value
            elif value.is_bool_constant():
                solution.addVariable(variable, bool(value.constant_value()))

# Fallback (model completion safety)
            else:
                value = self.solver.get_value(variable.expression, model_completion=True)

                if value.is_bv_constant():
                    solution.addVariable(variable, value.constant_value())
                elif value.is_bool_constant():
                    solution.addVariable(variable, bool(value.constant_value()))
                else:
                    raise RuntimeError(f"Unassigned SMT variable: {variable} -> {value}")
