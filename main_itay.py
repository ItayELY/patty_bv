...
def main():
    ...
    try:
        ...
        bound = args.bound if args.bound else 2
        ...
        order_tag = order.copy()
        # pattern2 = Pattern.fromOrder(order2)
        # pattern2.__order = order
        ...
        while bound <= bMax:
            pattern: Pattern = Pattern.fromOrder(order_tag)
            ...
            for f in order:
                    ...
                    new_f.name = f.name + f'_{bound}'
                    # f.planName = f.planName[:-1] + f'_{bound}' + ')'
                    order_tag.append(new_f)
                    gDomain.actions.add(new_f)
            bound += 1
